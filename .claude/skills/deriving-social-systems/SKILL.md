---
name: deriving-social-systems
description: >-
  Derives social and coordination structures from a species' biology, cognition,
  and environment rather than starting from a template of expected institutions.
  Works for any species — alien civilizations derive non-human structures; human
  civilizations in novel environments derive structures that may differ from any
  Earth precedent. Triggers on "derive social systems", "what structures emerge",
  "how do they coordinate", "derive culture from biology", "what would their
  society look like", "build their social structure", or during Step 4 of the civilization-builder skill. Produces the derivation analysis only — does NOT write
  final files.
---
...
2. **In-species name:** What the species would call it, once language
   exists. (May need to wait for the language phase of the civilization-builder skill.)
...
`.work/{project-name}/social-derivation-[civilization].md` (project-name derived from PROJECT_ROOT). When approved, hand it to **bible-builder** to fold into `{PROJECT_ROOT}/bible/universe/`.
...
Does NOT write final civilization files (use **bible-builder**).
Does NOT design planets or biology (consult `craft/references/designing-worlds.md`).
Does NOT audit for imported assumptions after the fact (use the **world-assumptions-reviewer** agent).
